...
import poke.server.image.ImageInitializer;
import poke.server.image.ImageQueue;
...
public class Server {
	 ...
	 *            The port to listen to
	 */
	private static class StartImage implements Runnable {
		private ServerConf conf;

		public StartImage(ServerConf conf) {
			this.conf = conf;
		}
		...
		public void run() {
			...
			try {
				ServerBootstrap b = new ServerBootstrap();
				bootstrap.put(conf.getImgPort(), b);
				...
				boolean compressComm = false;
				b.childHandler(new ImageInitializer(compressComm));

				// Start the server.

				logger.info("Starting image " + conf.getNodeId() + ", listening on port = " + conf.getImgPort());
				ChannelFuture f = b.bind(conf.getImgPort()).syncUninterruptibly();
	...
	private void startManagers() {
		...
		//LOG Manager
		logMgr = LogManager.initManager(conf);
	...
	public void run() {
		...
		StartImage img = new StartImage(conf);
		Thread ithread = new Thread(img);
		ithread.start();
