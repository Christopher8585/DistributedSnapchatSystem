...
public class ConnectionManager {
	...
	//For sending request to a node
	public synchronized static void sendTo(Request req, int to) {
		if (req == null)
			return;

		connections.get(to).write(req);
	}
	
	//For sending mgmt to a node
	public synchronized static void sendTo(Management mgmt, int to) {
		if (mgmt == null)
			return;
		
		mgmtConnections.get(to).write(mgmt);
	}
