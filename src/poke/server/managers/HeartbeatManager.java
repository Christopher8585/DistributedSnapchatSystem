...
public class HeartbeatManager extends Thread {
	...
	public static HeartbeatManager initManager(ServerConf conf) {
		...
		instance.compareAndSet(null, new HeartbeatManager());
		 Random r=new Random();
		sHeartRate = 8000 + r.nextInt(5) *2000; //############
		return instance.get();
	}
	...
	private Management generateHB() {
		...
		h.setTerm(ElectionManager.getInstance().getTerm());
		
		if(InboundMgmtWorker.processing)
			h.setIndex(LogManager.getInstance().getIndex());
		else
			h.setIndex(-2);
		
		
		h.setPrevTerm(LogManager.getInstance().getPrevTerm());//Not Reqd
		h.setPrevIndex(LogManager.getInstance().getPrevIndex());
		
		if(!LeadersPing && !isLeader)
			ElectionManager.getInstance().leaderNode=null;
		
		if(ElectionManager.getInstance().leaderNode==null)
			h.setLeaderNode(-1);
		else
			h.setLeaderNode(ElectionManager.getInstance().leaderNode);
	...
	private static boolean LeadersPing=false;
	private static boolean isLeader=false;
	
	public static void setIsLeader(boolean b){
		isLeader=b;
	}
	
	public static void setLeaderExists(boolean b){
		LeadersPing=b;
	}
	
	public static void setSHeartRate(int msec){
		sHeartRate=msec;
	}
		...
		while (forever) {
			try {
				...
				if(isLeader)
					...
					setSHeartRate(10000 + randomInt*1000);
				
				if(!LeadersPing && !isLeader && (ElectionManager.getInstance().leaderNode==null || ElectionManager.getInstance().leaderNode==-1)
						&& ElectionManager.electionStateFlag ){
				...
				System.out.println( conf.getNodeId()+" RUNNING "+ LeadersPing + " "+isLeader);
				...
				if (outgoingHB.size() > 0) {
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						//Send Ack only to the leader
						if(LeadersPing && !isLeader){
							 if(hd.getNodeId()!= ElectionManager.getInstance().whoIsTheLeader()){
									 continue;
							 }
						}
